...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_POST['id_type'])) {
        switch ($_POST['id_type']) {
            case 'national_id':
                $national_id = $_POST['id_value'] ?? null;
                break;
            case 'passport':
                $passport_number = $_POST['id_value'] ?? null;
                break;
            case 'driver_license':
                $driver_license = $_POST['id_value'] ?? null;
                break;
            case 'voter_id':
                $voter_id = $_POST['id_value'] ?? null;
                break;
        }
    }
        ...
        <form method="POST" id="customerForm" enctype="multipart/form-data">
            ...
            <div class="form-section">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        <label class="form-label required-field">Primary Phone</label>
                        <input type="text" 
                               name="phone" 
                               class="form-control" 
            ...
            <div class="form-section">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <select name="id_type" id="id_type" class="form-select" onchange="toggleIDField()">
                    ...
                    <div class="col-md-6" id="id_value_field">
                        <label class="form-label" id="id_value_label">ID Number</label>
                        <input type="text" 
                               name="id_value" 
                               id="id_value"
                               class="form-control" 
                               placeholder="Enter ID number"
                               value="<?php echo htmlspecialchars($_POST['id_value'] ?? ''); ?>">
                    </div>
            ...
            <div class="form-section">
                ...
                <div class="row g-3">
                    <div class="col-md-12">
                        <h6 class="text-muted mb-3">Guardian 1</h6>
                    </div>
                    ...
                    <div class="col-md-12">
                        <hr>
                        <h6 class="text-muted mb-3">Guardian 2</h6>
                    </div>
...
// Toggle ID field based on selected ID type
function toggleIDField() {
    const idType = document.getElementById('id_type').value;
    const idValueField = document.getElementById('id_value_field');
    const idValueLabel = document.getElementById('id_value_label');
    const idValueInput = document.getElementById('id_value');
    
    if (idType) {
        idValueField.style.display = 'block';
        
        // Update label and placeholder based on ID type
        switch(idType) {
            case 'national_id':
                idValueLabel.textContent = 'National ID (NIDA) Number';
                idValueInput.placeholder = 'Enter NIDA number';
                break;
            case 'passport':
                idValueLabel.textContent = 'Passport Number';
                idValueInput.placeholder = 'Enter passport number';
                break;
            case 'driver_license':
                idValueLabel.textContent = 'Driver License Number';
                idValueInput.placeholder = 'Enter driver license number';
                break;
            case 'voter_id':
                idValueLabel.textContent = 'Voter ID Number';
                idValueInput.placeholder = 'Enter voter ID number';
                break;
        }
    } else {
        idValueField.style.display = 'none';
        idValueInput.value = '';
    }
}
